...
export class DataSourcesService {
    ...
    private async getConfig(alias: string): Promise<DataSourceConfigInterface> {
        ...
        if (!item) throw `DataSource '${alias}' not found`
    ...
    async exportData(alias: string, params: ExportParams, context: Context) {
        ...
        for (let i in params.fields) {
            ...
            if (field) titles.push(field.title)
        }
        sheet.push(titles)
        ...
        switch (params.format) {
            case 'xlsx':
                return arrayToExcel(sheet)
            case 'csv':
                return arrayToCsv(sheet)
            case 'json':
                return Buffer.from(
